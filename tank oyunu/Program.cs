...
namespace tank_oyunu
{
	...
	class Program
	{
		...
		public class tank
		{
			...
			public static int tanksayisi=0;
			public int yon=7;
			public konum yer;
			public bool attımi=false;
			public tank()
			{
				no=tanksayisi;
				tanksayisi++;
				yer.x=r.Next(1,boy-1);
				yer.y=r.Next(1,boy-1);
				
			}
			public void hareket(int yon)
			{
				switch(yon){
					case '2':
						if (harita[yer.x + 1, yer.y] == 0)
						{
							harita[yer.x,yer.y]=0;
							yon=harita[yer.x +=1, yer.y] = 2;
						}
						break;

					case '4':
						if (harita[yer.x, yer.y - 1] == 0)
						{
							harita[yer.x,yer.y]=0;
							yon=harita[yer.x, yer.y-=1] = 4;
						}
						break;
						case '5':
						if(!attımi)
		...
		public class bomba:tank
		{
			...
			public void hareket()
			{
				switch(yön){
					case 2:
						if(harita[y.x+1,y.y]==0){
							harita[y.x,y.y]=0;
							harita[y.x+=1,y.y]=9;
							
						}else if(harita[y.x+1,y.y]!=1)
						{
							harita[y.x,y.y]=0;
							harita[y.x+1,y.y]=0;
						}else harita[y.x,y.y]=0;break;
						case 4:if(harita[y.x,y.y-1]==0){
							harita[y.x,y.y]=0;
							harita[y.x,y.y-=1]=9;
						}else if(harita[y.x,y.y-1]!=1)
						{
							harita[y.x,y.y]=0;
							harita[y.x,y.y-1]=0;
						}else harita[y.x,y.y]=0;break;
						case 6:if(harita[y.x,y.y+1]==0){
							harita[y.x,y.y]=0;
							harita[y.x,y.y+=1]=9;
						}else if(harita[y.x,y.y+1]!=1)
						{
